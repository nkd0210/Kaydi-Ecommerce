...
const Chat = () => {
    ...
    useEffect(() => {
        if (currentUser) {
            ...
            const handleNewSingleChat = (newChat) => {
                setAllChats((allChats) => {
                    // Check if the newChat already exists to avoid duplicates
                    if (!allChats.some(chat => chat._id === newChat._id)) {
                        return [newChat, ...allChats];
                    }
                    return allChats;
                });
            };
            ...
            pusherClient.bind("new-single-chat", handleNewSingleChat);
            pusherClient.bind("new-group-chat", handleNewGroupChat);

            return () => {
                pusherClient.unsubscribe(currentUser._id);
                pusherClient.unbind("update-chat", handleChatUpdate);
                pusherClient.unbind("new-single-chat", handleNewSingleChat);
                pusherClient.unbind("new-group-chat", handleNewGroupChat);
            };
        }
    }, [currentUser]);
                    ...
                    <div className="flex justify-between p-[20px]">
                        ...
                        <div className="flex items-center gap-[10px]">
                            ...
                            <div
                                ...
                                className="border rounded-[50%] w-[30px] h-[30px] flex items-center justify-center bg-[#212121] hover:bg-[#424242] hover:cursor-pointer "
                            >
                                                ...
                                                allChats?.map((chat, index) => (
                                                    <div key={index} className="">
                                                        ...
                                                        {chat.isGroupChat ? (
                                                            ...
                                                            <div
                                                                onClick={() => {
                                                                    setSelectId(chat?._id);
                                                                    setChatId(chat?._id);
                                                                    setOpenMainSidebar(false);
                                                                    handleAccessGroupChat(chat?._id);
                                                                    setSingleChat({});
                                                                }}
                                                                className={`flex justify-between items-center w-full py-[20px] border-b-[1px] h-[80px] border-gray-600 cursor-pointer hover:bg-[#292929] ${selectId === chat?._id ? "bg-gray-800" : ""
                                                                    }  `}
                                                            >
                                                                <div className="flex items-center gap-[20px] pl-[20px]">
                                                                    ...
                                                                    <div className="flex flex-col gap-[5px]">
                                                                        ...
                                                                        {chat?.latestMessage ? (
                                                                            <p
                                                                                className={`text-[11px] w-[150px] truncate ${chat?.latestMessage?.seenBy.find(
                                                                                    (member) =>
                                                                ...
                                                                {chat?.latestMessageAt ? (
                                                                    <p className="text-[11px] pr-[20px]">
                                                                        {formatDate(chat?.latestMessageAt)}
                                                                    </p>
                                                                ) : (
                                                            ...
                                                            chat.receiver && chat.receiver[0] && (
                                                                <div
                                                                    onClick={() => {
                                                                        setSelectId(chat?.receiver[0]?._id);
                                                                        setChatId(chat?._id);
                                                                        setOpenMainSidebar(false);
                                                                        handleAccessChat(chat?.receiver[0]?._id);
                                                                        setSingleGroupChat({});
                                                                    }}
                                                                    className={`flex justify-between items-center w-full py-[20px] border-b-[1px] h-[80px] border-gray-600 cursor-pointer hover:bg-[#292929] ${selectId === chat?.receiver[0]?._id
                                                                        ? "bg-gray-800"
                                                                        : ""
                                                                        }`}
                                                                >
                                                                    <div className="flex items-center gap-[20px] pl-[20px]">
                                                                        <div className="w-[50px]">
                                                                            <img
                                                                                key={index}
                                                                                src={chat?.receiver[0]?.profilePic}
                                                                                alt="ava"
                                                                                className="w-[40px] h-[40px] object-cover rounded-[50%]"
                                                                            />
                                                                        </div>
            ...
            <Modal open={openModal} onClose={() => setOpenModal(false)}>
                <div className="absolute top-[50%] left-[50%] transform translate-x-[-50%] translate-y-[-50%] shadow-lg w-[500px] bg-[#232323] text-black h-[400px] overflow-hidden overflow-y-scroll max-md:h-[200px] rounded-[10px]">
                    <CreateGroupChat
                        ...
                        setSelectId={setSelectId}
                        setChatId={setChatId}
                        setLoadingChatBox={setLoadingChatBox}
                        setOpenMainSidebar={setOpenMainSidebar}
                    />
                </div>
            </Modal>
